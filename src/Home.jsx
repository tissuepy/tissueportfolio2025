...
import citylineImage from './assets/cityline.png';
...
function Home() {
  ...
  useEffect(() => {
    ...
    // Cityline fades in after hero text finishes
    const citylineTimeout = setTimeout(() => {
      const cityline = document.querySelector('.cityline-container');
      if (cityline) cityline.classList.add('fade-in-visible');
    }, 1200);
    ...
    const observeFadeElements = () => {
      const fadeElements = document.querySelectorAll('.fade-in-on-scroll:not(.hero-section):not(.hero-text):not(.cityline-container)');
  ...
  return (
    <>
          ...
          <div className="hero-content hero-content-centered">
            <div className="hero-text hero-text-centered fade-in-on-scroll" ref={heroTextRef}>
              <p className="hero-subtitle">
                Nitish Gannu is a Product Designer currently designing for 3 million users at <a href="https://www.joinpogo.com/" target="_blank" rel="noopener noreferrer" className="hero-link hero-link-pogo">Pogo<sup>1</sup></a> and studying data science and statistics at <a href="https://www.cornell.edu" target="_blank" rel="noopener noreferrer" className="hero-link hero-link-cornell">Cornell<sup>2</sup></a>
              </p>
              <p className="hero-previously">
                Previously designed for <a href="https://openai.com/chatgpt" target="_blank" rel="noopener noreferrer" className="hero-link hero-link-chatgpt">ChatGPT<sup>3</sup></a>, <a href="https://trywrap.com/" target="_blank" rel="noopener noreferrer" className="hero-link hero-link-wrap">Wrap<sup>4</sup></a>, then <a href="https://thinkneuro.org/" target="_blank" rel="noopener noreferrer" className="hero-link hero-link-thinkneuro">ThinkNeuro<sup>5</sup></a>
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Cityline banner image */}
      <div className="cityline-container fade-in-on-scroll">
        <img src={citylineImage} alt="Nitish Gannu cityline" className="cityline-image" />
        <div className="cityline-stars">
        ...
        {showWallet ? (
          <div className="main-content-container">
            <div className="wallet-demo-section" style={{ paddingTop: '80px', paddingBottom: '150px', paddingLeft: '80px' }}>
              <WalletCard />
            </div>
          </div>
        ) : (
